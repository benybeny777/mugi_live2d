...
- Photoshop 2026でレイヤー保持のまま `mugi-hiyori-compatible-final-2x.psd`（5952×8350）を作成した。
- 18個の透過パーツを最大8192pxでVRMへ格納する。形状・リグ・Expression・Spring Boneは安定版から変更しない。
- 旧2976×4175 PSDは比較・復旧用に保持する。
- 高解像度化は線と境界の再サンプル品質を改善するもので、新しい絵柄や3Dディテールを生成する処理ではない。
- 8192テクスチャ非対応環境では旧4096版を利用する。
