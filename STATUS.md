...
## 2026-08-13 VRM画像高解像度化

- Adobe Photoshop 2026 の自動再サンプルで、VRM正本PSDを `2976×4175` から `5952×8350` へ2倍化した。
- 元PSDを残し、53レイヤー・透明度・合成結果を独立再読込で確認した。
- VRMのテクスチャ上限を4096から8192へ変更。18メッシュ、459頂点、Expression 17+4種、Spring Bone 3チェーン/5ジョイントは維持した。
- 固定ピクセルだった脚・胴体の分割座標と口内変形の基準高さをキャンバス比率へ修正し、2倍キャンバスの回帰テストを追加した。
- 同条件PNG比較で構図と色を維持し、目・髪・服の線がわずかに明瞭になることを確認した。正式VRMは約9.98 MBで10 MiB以内。
- 実写テストは別出力で行う。素材未指定のため未着手で、ユーザー所有画像をローカル限定で扱う。
