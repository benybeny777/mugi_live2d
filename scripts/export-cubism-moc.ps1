...
$hashInput=((Get-FileHash (Join-Path $source 'CubismMocExportAgent.java') -Algorithm SHA256).Hash+(Get-FileHash (Join-Path $source 'CubismMocExportAgentV3.java') -Algorithm SHA256).Hash)
...
@"
Agent-Class: mugi.bridge.CubismMocExportAgentV3

"@|Set-Content $manifest -Encoding ascii
& (Join-Path $jdk 'javac.exe') -encoding UTF-8 -cp "$cubismLib\*" -d $classes (Join-Path $source 'AttachAgent.java') (Join-Path $source 'CubismMocExportAgent.java') (Join-Path $source 'CubismMocExportAgentV3.java')
...
if(Test-Path "$resolved.error.txt"){Remove-Item -LiteralPath "$resolved.error.txt" -Force}
